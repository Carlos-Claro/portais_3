...
//			$this->connect->close();
		...
		if(isset($this->config[$this->activate]) == TRUE)
		{
			...
			else
			{
                            if (is_array($this->config[$this->activate]['hostname'])) {
                                $this->hostname = [];
                                foreach ($this->config[$this->activate]['hostname'] as &$hostname) $this->hostname[] = trim($hostname);
                            } else
			...
			$dsn = "mongodb://";
                        if (is_array($this->hostname)) {
                            ...
                            foreach ($this->hostname as $hostname) {
                                $dsn .= $comma."{$hostname}:{$this->port}"; $comma = ',';
                            }
                        } else 
	...
	*/
	...
	{		
		...
		try{	
                        $options = ['limit'=>(int) $this->limit, 'skip'=>(int) $this->offset, 'sort'=>$this->sorts, 'batchSize'=>(int)$this->limit, 'cursorType'=>1, 'projection'=>$this->selects];
