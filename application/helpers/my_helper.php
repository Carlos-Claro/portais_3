...
function form_selecionavel($item, $selecionado = NULL, $link = TRUE, $valor = TRUE)
{
    ...
    foreach( $item['valor'] as $data )
    {
        if ( isset($data->qtde) )
        {
            ...
            $retorno .= (isset($data->qtde) && $data->qtde > 0 && $valor) ? '<span class="badge pull-right">' . $data->qtde . '</span>' : '';
            $retorno .= '</a>';
        }
...
function set_existe_image($id = FALSE, $tipo = 'destaque', $id_empresa = NULL, $arquivo = FALSE, $sequencia = 0 )
{
    $a = FALSE;
    $nao_tem_sequencia = ( $tipo == 'TM' || $tipo == 'destaque_home' ) ? TRUE : FALSE;
...
function set_arquivo_image( $id, $arquivo, $id_empresa, $mudou = FALSE, $fs = '', $sequencia = 1, $tipo = 'TM')
{
    ...
    $array_tamanho = array(
                            ...
                            'destaque' => array('width' => '280', 'height' => '200', 'crop' => FALSE),
    ...
    else
    {
        ...
        else
        {
            if ( $tipo == '650F')
            {
                $a = str_replace('codEmpresa', $id_empresa, URL_IMAGE_MUDOU);
                $a .= $tipo.'_'.$arquivo;
//                $a .= $tipo.'_F_'.$id.( $nao_tem_sequencia ? '' : '_'.$sequencia ).'.';
//                $a .= str_replace('.','',substr($arquivo, -4));
            }
            elseif ( $tipo == 'destaque' )
            {
                $a = str_replace('codEmpresa', $id_empresa, URL_IMAGE_MUDOU);
                $a .= $arquivo;
                //$arquivo_curl = curl_executavel($a);
...
function curl_executavel( $endereco, $header = 'UTF-8')
{
    $endereco = str_replace('https', 'http', $endereco );
//    var_dump($endereco);die();
    $ch = curl_init();
//    curl_setopt( $ch, CURLOPT_HTTPHEADER , array( 'Content-type: ' . image_type_to_mime_type($endereco).';charset="UTF-8";' ) );
...
function deletar_pasta( $caminho, $pasta )
{
    $endereco = $caminho.$pasta;
    if ( is_dir($endereco)  )
    {
        $end_mascara = $endereco.'/*.*';
...
function tira_html($valor, $conjunto = array() )
{
    $strip = strip_tags($valor, '<p><br>');
    $telefone = isset($conjunto[0]) ? $conjunto[0] : NULL;
    $com_espaco = '';
    $qtde = strlen($telefone);
    for( $i = 0; $i < $qtde; $i++ )
    {
        $com_espaco .= $telefone[$i].' ';
    }
    $conjunto[] = $com_espaco;
    $conjunto[] = 'whatsapp';
    $conjunto[] = 'Whatsapp';
    $conjunto[] = 'facebook';
    $conjunto[] = 'facebook.com';
    $conjunto[] = 'fb.com';
    $conjunto[] = 'com.br';
    $conjunto[] = 'Edisonborges';
    $conjunto[] = 'Edisonborges.';
    $conjunto[] = 'EdisonBorges.';
    $conjunto[] = 'VendeImoveisCuritiba.com.br';
    $conjunto[] = 'VendeImoveisCuritiba';
    $conjunto[] = 'vendeimoveiscuritiba';
    $conjunto[] = 'EdisonBorges.com.br';
    $retorno = str_replace($conjunto, '', $strip);
    $expressoes = array(
        '(\([2-9][1-9]\) [5-9][0-9]{3}-[0-9]{4})',
        '(\([2-9][1-9]\) [4-9][0-9]{3}-[0-9]{4})',
        '(\([2-9][1-9]\)[4-9][0-9]{3}-[0-9]{4})',
        '(\([2-9][1-9]\)[4-9][0-9]{3}.[0-9]{4})',
        '(\([2-9][1-9]\)[4-9][0-9]{3}.[0-9]{4})',
        '(\([0-9][1-9]\))',
    ...
    $retorno = preg_replace($expressoes, '***', $retorno);
    return $retorno;
}